...
from untangling.utils.grasp import Grasp
import time
...
class Interface:
    ...
    def __init__(
        ...
        self.T_PHOXI_BASE = RigidTransform.load(
            "/home/justin/yumi/phoxipy/tools/phoxi_to_world_bww.tf"
        ...
        self.before_after_poses_dir = '/home/justin/yumi/cable-untangling/untangling/utils/debug_motions/before_after_poses'
        self.successful_motion_dir = '/home/justin/yumi/cable-untangling/untangling/utils/debug_motions/successful_motions'

    def get_frame(self, which_arm):
        return self.yk.l_tcp_frame if which_arm == "left" else self.yk.r_tcp_frame

    def take_image(self, cam_only=False):
    ...
    def go_delta(self, l_trans=None, r_trans=None, reltool=False, linear=True):
    ...
    def partial_pull_apart(self, dist, slide_left = True,slide_right=True,tilt=False, layout_nicely=False):
        ...
        lp = RigidTransform(
            translation=[0.25, min(dist+left_point-left_exceed+right_exceed, MAX_DIST), 0.15],
            rotation=l_tar_R,  # 0.45 (also below)
            from_frame=YK.l_tcp_frame,
            to_frame="base_link",
        )
        ...
        if r_R[:, 1].dot(np.array((1, 0, 0))) > 0:  # if y axis points towards pos x
            ...
            r_tar_R = RigidTransform.z_axis_rotation(-np.pi/2)@self.GRIP_DOWN_R
        rp = RigidTransform(
            translation=[0.25, max(right_point-dist+right_exceed-left_exceed, -MAX_DIST), 0.15],
            rotation=r_tar_R,
            from_frame=YK.r_tcp_frame,
            to_frame="base_link",
        )
        ...
        if layout_nicely:
            ...
            while (not success and target_x > 0.3):
                try:
                    ...
                    lp.translation[1] *= 0.5
                    rp.translation[1] *= 0.5
                    self.go_cartesian(
                        l_targets=[lp],
                        r_targets=[rp],
                        nwiggles=(1, 1),
    ...
    def reveal_endpoint(self, img, endpoints, g, closest_to_pos=None):
        ...
        self.grasp(l_grasp=grasp) if arm_to_use == 'left' else self.grasp(r_grasp=grasp)
        ...
        if arm_to_use == 'right':
            rp1 = RigidTransform(
                translation=[(grasp.pose.translation[0] - 0.35) * -1/3 + 0.35, grasp.pose.translation[1] * 1/4, 0.1],
                rotation=self.GRIP_DOWN_R, #RigidTransform.x_axis_rotation(-.4)@
                from_frame=YK.r_tcp_frame,
                to_frame="base_link"
            )
            try:
