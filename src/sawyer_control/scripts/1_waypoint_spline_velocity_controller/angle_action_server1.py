...
class PositionController():
    def __init__(self, control_rate):
        ...
        self.gripper.set_cmd_velocity(0.001)
    ...
    def update_plan(self, waypoints, duration=1.5, reset=False, gripper=1):
        """
    ...
    def reset(self):
        ...
        data = pickle.load(open("/home/tesla/ros_ws/reset.p", "rb")) #pickle.load(open("/home/ashvin/ros_ws/reset2.p", "rb"))
        joints = data["joints"]
...
def execute_action(action_msg):
    angles = action_msg.angles
    if len(angles) == 7:
        action = action_msg.angles
        gripper = 1
    else:
        action = action_msg.angles[:7]
        gripper = action_msg.angles[7]
    ...
    if joint_to_values:
        ...
        controller.update_plan([ja], duration=duration, reset=reset, gripper=gripper)
