...
def compute_joint_angle(req):
    ee_pos = req.ee_pos
    curr_angles = req.curr_joint_angles
    curr_angles = dict(zip(joint_names, curr_angles))
    Q = Quaternion(
        x=ee_pos[3],
        y=ee_pos[4],
        z=ee_pos[5],
    ...
    ik_angles = get_joint_angles(pose, curr_angles, True, False) #  use_advanced_options=True, current=False
